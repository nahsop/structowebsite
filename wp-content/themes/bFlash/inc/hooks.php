 ...
 */ {
	function _action_theme_setup() {
		...
		fw_theme_thumbnail_sizes();
	}
}
endif;
add_action( 'init', '_action_theme_setup' );

function fw_theme_thumbnail_sizes(){
		$sizes = [
			'thumb370' =>[ 370, 270 ],
			'thumb570' =>[ 570, 500 ],
			'thumb185' =>[ 185, 100 ],
			...
			'thumb400' =>[ 400, 300 ],
			'thumb1920' =>[ 1920, 400 ],
			'thumbBanner' =>[ 1920, 900 ],
			'thumbSlider' =>[ 1920, 810 ],
			'thumbHero' =>[ 1920, 600 ],
			'thumb99' => [99, 71],
			'thumbSliderOne' => [300,300],
			'thumbSliderTwo' => [270,270],
			'thumbArchveBig' => [800,400],
			'thumbPortrait' => [480,790],
			// 'thumbPostSlider' => [],
			'thumbPostSliderThumb' =>[170, 100],
			
		];
		array_walk($sizes, function($value, $key){
			$width =  $value[0];
			$height = $value[1];
			$crop = true;
...
function _action_theme_widgets_init() {
	...
	$sidebars = [
		array(
			'name'          => __( 'Footer Column 1', 'unyson' ),
			'id'            => 'footer-1',
			'description'   => __( 'Appears in the footer section of the site.', 'unyson' ),
			// 'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			// 'after_widget'  => '</aside>',
		...
		array(
			'name'          => __( 'Footer Column 2', 'unyson' ),
			'id'            => 'footer-2',
			'description'   => __( 'Appears in the footer section of the site.', 'unyson' ),
			// 'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			// 'after_widget'  => '</aside>',
		...
		array(
			'name'          => __( 'Footer Column 3', 'unyson' ),
			'id'            => 'footer-3',
			'description'   => __( 'Appears in the footer section of the site.', 'unyson' ),
			// 'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			// 'after_widget'  => '</aside>',
		...
		array(
			'name'          => __( 'Footer Column 4', 'unyson' ),
			'id'            => 'footer-4',
			'description'   => __( 'Appears in the footer section of the site.', 'unyson' ),
			// 'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			// 'after_widget'  => '</aside>',
		...
		array(
			'name'          => __( 'Main Sidebar', 'unyson' ),
			'id'            => 'main-sidebar',
			'description'   => __( 'Appears in the Right of the site.', 'unyson' ),
			// 'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			// 'after_widget'  => '</aside>',
			// 'before_title'  => '<h1 class="widget-title">',
			// 'after_title'   => '</h1>',
...
function fw_header_left_add_search($items) {
	////////////////////
	// return $items; //
	////////////////////
	// Get search box setting
	if(fw_get_db_customizer_option('header_style') != 'left'){ return $items; }
	$enable_search = fw_get_db_settings_option('search_box');
	// Check if search box is enabled
	if($enable_search == 'no') return $items;
  	return $items .= '<li class="item-search">
                            <span class="btn-search hidden-xs hidden-sm" data-toggle="collapse" data-target="#box-search-header"><i class="fa fa-search"></i></span>

                        </li>';
}
add_filter('wp_nav_menu_items','fw_header_left_add_search');
